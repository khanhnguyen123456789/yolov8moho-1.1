...
const ExportIcon = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" /></svg>;
...
export const Timeline: React.FC = () => {
    ...
    const [copyStatus, setCopyStatus] = useState('Copy Lua');
    ...
    const getPlaceholderValue = (layerId: string): string => {
        if (layerId.includes('translate')) return '{x=0, y=0}';
        if (layerId.includes('scale')) return '{x=1, y=1}';
        if (layerId.includes('rotate')) return '0';
        if (layerId.includes('shear')) return '{x=0, y=0}';
        if (layerId.includes('camera')) return 'true';
        return '0';
    };

    const generateLuaScript = (): string => {
        const luaLines: string[] = [
            "-- Auto-generated Lua keyframes for Moho Studio Pro",
            "-- Generated by Yolov8Moho Plugin\n"
        ];
    
        layers.forEach(layer => {
            if (layer.keyframes.length > 0) {
                luaLines.push(`-- Keyframes for: ${layer.name}`);
                layer.keyframes.forEach(frame => {
                    const value = getPlaceholderValue(layer.id);
                    luaLines.push(`Moho.SetKey("${layer.name}", ${frame}, ${value})`);
                });
                luaLines.push("");
            }
        });
    ...
    const handleExport = () => {
        const scriptContent = generateLuaScript();
        const blob = new Blob([scriptContent], { type: 'text/lua' });
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.download = 'timeline_export.lua';
    ...
    const handleCopy = () => {
        ...
        navigator.clipboard.writeText(scriptContent).then(() => {
            ...
            setTimeout(() => setCopyStatus('Copy Lua'), 2000);
        }).catch(err => {
                ...
                <div className="flex items-center justify-between text-xs text-gray-400 mb-2 px-2">
                    <span>Click to select. Drag to move. <span className="font-semibold text-teal-300">Alt+Drag</span> to duplicate.</span>
                     <div className="flex items-center space-x-2">
                        <button onClick={handleExport} className="flex items-center space-x-1 px-2 py-1 rounded bg-gray-700 hover:bg-gray-600 transition-colors" title="Export to .lua file"><ExportIcon /><span>Export .lua</span></button>
                        <button onClick={handleCopy} className="flex items-center space-x-1 px-2 py-1 rounded bg-gray-700 hover:bg-gray-600 transition-colors" title="Copy Lua script to clipboard">
